...
CONNECTOR_PRESETS = [
    ...
    {"id": "puppeteer", "name": "Puppeteer(浏览器自动化)", "transport": "stdio",
     "command": "npx", "args": ["-y", "@modelcontextprotocol/server-puppeteer"],
     "env_keys": [], "note": "无头浏览器访问/截图;建议开启「每次调用需审批」"},
    {"id": "gitlab", "name": "GitLab", "transport": "stdio",
     "command": "npx", "args": ["-y", "@modelcontextprotocol/server-gitlab"],
     "env_keys": ["GITLAB_PERSONAL_ACCESS_TOKEN", "GITLAB_API_URL"], "note": "自建 GitLab 填 API URL"},
    {"id": "sentry", "name": "Sentry(错误监控)", "transport": "stdio",
     "command": "uvx", "args": ["mcp-server-sentry", "--auth-token", "YOUR_TOKEN"],
     "env_keys": [], "note": "把 args 里 YOUR_TOKEN 换成 Sentry auth token"},
